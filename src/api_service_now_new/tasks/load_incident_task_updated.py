...
class LoadIncidentTaskUpdated(MixinGetDataset, Pipeline):
    """Carrega incident_task do ServiceNow e atualiza registros existentes por sys_id."""
    ...
    def run(self) -> Dict:
        ...
        self.load(dataset=self.dataset, model=IncidentTask)
    ...
    def _incident_task(self) -> pl.DataFrame:
        ...
        query = f"opened_at>={start_ts} 00:00:00^opened_at<={end_ts} 23:59:59^assignment_groupSTARTSWITHvita^ORassignment_groupSTARTSWITHvivo b2b centro servi"
...
@shared_task(
    name="api_service_now_new.load_incident_task_updated_async",
    bind=True,
    autoretry_for=(Exception,),
    retry_backoff=5,
    retry_kwargs={"max_retries": 3},
)
def load_incident_task_updated_async(_task, start_date: str, end_date: str):
    sync_task = LoadIncidentTaskUpdated(
