...
class LoadInterfaceVGR(MixinViews, Pipeline):
    ...
    def _get_solar_interface_original_queryset(self) -> QuerySet:
    ...
    def extract_transform_dataset(self) -> None:
        ...
        self.dataset = (
            self._solar_interface_original.pipe(self._limpar_texto)
    ...
    @property
    def _solar_interface_original(self) -> pl.DataFrame:
        ...
        return self.get_dataset(
            query_set=self._get_solar_interface_original_queryset().values(
                *schema
            ),
            schema=schema,
        )
    ...
    def _delete(self):
        ...
        n_deleted, __ = (
            SolarInterface.objects.using("power_bi")
            .filter(**self._filtro)
            .delete()
        )
    ...
    def _save(self):
        ...
        objs = [SolarInterface(**vals) for vals in self.dataset.to_dicts()]
        bulk = SolarInterface.objects.using("power_bi").bulk_create(
